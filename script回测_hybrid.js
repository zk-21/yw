...
function predictLikelyTailsV4Enhanced(sourceTails, transData, refRows, sourceRow, allBalls = null) {
  // 🆕 第一次优化权重（效果最好：Top5命中率33.5%，联合覆盖率64.3%，候选池覆盖率88.8%）
  // 核心优化：降低sameOrNeighbor权重（35→15），启用globalFreq（15），添加多样性保障机制
  const weights = {
    overlap1: ABLATION_CONFIG.disable_overlap1 ? 0 : 4,      // 降低：避免与sameOrNeighbor重叠
    arith1: ABLATION_CONFIG.disable_arith1 ? 0 : 6,        // 降低：等差模式只有6.7%
    overlap10: 2,     // 保持：远期参考影响较小
    arith10: 4,       // 降低：等差模式只有6.7%
    overlapBonus: 2,  // 保持：双重保留更可靠
    globalFreq: 15,   // 🆕 启用：平衡全局频率，避免预测偏差
    comboTransfer: ABLATION_CONFIG.disable_comboTransfer ? 0 : 2, // 保持：完全匹配率0%
    partialMatch: 8,  // 降低：为其他信号腾出空间
    highFreqMiss: 6,  // 降低：高频被漏球尾号
    sameOrNeighbor: ABLATION_CONFIG.disable_sameOrNeighbor ? 0 : 15, // ⚠️ 大幅降低：避免过度依赖前一期（35→15）
    crossRowTail: ABLATION_CONFIG.disable_crossRowTail ? 0 : 10,   // 降低：跨行尾号
    intraGroupPattern: ABLATION_CONFIG.disable_intraGroupPattern ? 0 : 12, // 降低：组内尾号规则
    arithmeticNumber: ABLATION_CONFIG.disable_arithmeticNumber ? 0 : 8, // 降低：完整号码等差
    crossRowArithmetic: ABLATION_CONFIG.disable_crossRowArithmetic ? 0 : 12, // 大幅降低：跨行等差（24→12）
    bridgeTails: ABLATION_CONFIG.disable_bridgeTails ? 0 : 8,        // 降低：桥接尾号
    tailClustering: 8,       // 降低：尾号聚类规律
    tailTransition: 10,      // 降低：尾号转换规律
    tailPeriodicity: 8,      // 降低：尾号周期性规律
    tailInterval: 8,         // 降低：尾号间隔规律
    tailOddEvenSize: 10,     // 保持：奇偶大小组合（增加多样性）
    tailArithmeticExtension: 12, // 降低：等差延伸邻号规律（18→12）
  };
  ...
  // 当期尾号中的等差对（如7,9），延伸点（如5）及其邻号（如4,6）在下期出现概率极高
  if (allBalls && sourceRow > 0) {
  ...
  // 确保Top5预测包含足够的偶数尾号，并平衡所有尾号的预测机会
  const sortedScores = [...scores.entries()].sort((a, b) => b[1] - a[1]);
...
function updatePredictionDisplay(predictedTails, ivPrediction, firstBallPredictions, sourceRow, selectedNumbers, _preCollected, _candidateEntries) {
  ...
  if (predictedTails && predictedTails.length > 0) {
    ...
    tailPredictionDiv.innerHTML = `
      ...
      <div class="prediction-info">基于9种尾号模式分析 → 实际号码组合</div>
    `;
  } else {
...
if (__isNode) {
  ...
  for (let sourceIdx = 1; sourceIdx <= totalDraws - PREDICT_INTERVAL - 1; sourceIdx++) {
    ...
    try {
      ...
      // 调试：输出多段连续模式数量
      if (sourceIdx <= 5) {
        console.log(`[DEBUG] sourceIdx=${sourceIdx} sourceRow=${sourceRow} msPatterns=${msPatterns.length} top=${JSON.stringify(msPatterns.slice(0,3))}`);
        console.log(`[DEBUG] allCombos.length=${allCombos.length} poolNumbers.length=${poolNumbers.length}`);
      }
      ...
      if (msPatterns.length > 0) {
        ...
        for (const [pattern, count] of msPatterns) {
          ...
          // 调试
          if (sourceIdx <= 3) {
            console.log(`[DEBUG] pattern=${pattern} baseTails=${baseTails} result=${result} msTailsSet=${[...msTailsSet]}`);
            console.log(`[DEBUG] allCombos[0] keys=${Object.keys(allCombos[0]||{})}`);
            if (allCombos[0]) {
              console.log(`[DEBUG] allCombos[0].numbers=${allCombos[0].numbers} allCombos[0].key=${allCombos[0].key}`);
            }
          }
          ...
          // 调试
          if (sourceIdx <= 3 && scored.length > 0) {
            console.log(`[DEBUG] top scored combo: msScore=${scored[0].msScore} numbers=${scored[0].numbers}`);
          }
      ...
      // 调试：输出最终选择结果
      if (sourceIdx <= 5) {
        console.log(`[DEBUG] FINAL selectedFront=${selectedFront.length} numbers=${selectedFront.map(c => (c.numbers||[]).join(',')).join(' | ')}`);
      }
      ...
      // 收集补漏6
      if (bl6) {
        const backNums = variantPlan[5]?.back?.numbers || [];
        predictionDetails.push({
          issue,
          sourceNums: sourceNumsStr,
          targetNums: targetNumsStr,
          predictionType: '补漏6',
          front1: bl6.numbers[0] || '',
          front2: bl6.numbers[1] || '',
          front3: bl6.numbers[2] || '',
          front4: bl6.numbers[3] || '',
          front5: bl6.numbers[4] || '',
          back1: backNums[0] || '',
          back2: backNums[1] || ''
        });
      }
  ...
  results.forEach(r => {
    ...
    if (r.predictedTails && r.targetTails) {
      ...
      const missedTails = targetTails.filter(t => !predTails.includes(t));
      ...
      console.log(`    漏掉尾号:       [${missedTails.join(', ')}]`);
...
if (__isNode && process.argv.includes('--compare-seeds')) {
  ...
  console.log("\n【固定种子测试】");
  console.log("-".repeat(90));
  const seeds = [12345, 67890, 11111, 54321, 99999, 123456, 88888, 77777, 33333, 55555];
  const seedResults = [];
