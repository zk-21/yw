...
const WRONG_CODE_RULES = {
  '阅读理解': {
    code: 'R2',
    label: '信息定位',
    train: '回到原文找依据，再用“原文线索 + 自己概括”回答。'
  },
  '句式转换': {
    code: 'S1',
    label: '句式结构',
    train: '先找动作发出者和承受者，再做把字句、被字句互换。'
  },
  '思维进阶': {
    code: 'W2',
    label: '表达展开',
    train: '补动作、语言、心理或原因，让答案从“说对”变成“说完整”。'
  },
  '词语选择': {
    code: 'V1',
    label: '语境辨析',
    train: '读完整句，比较词语感情色彩和搭配对象。'
  }
};
...
const SCENARIO_PATHS = [
  {
    ...
    title: '60分起步补弱路径',
    goal: '先把字词、审题和基础表达补稳。',
    tasks: ['每天10分钟字词听写', 'A卷只追3类高频错因', '错题3天后复练一次']
  },
  {
    tag: '中等生',
    title: '85分到95分瓶颈突破',
    goal: '减少会做但丢分的题，补齐答案完整度。',
    tasks: ['阅读题必须标原文依据', 'B卷复测同类问法', '每周整理2个满分模板']
  },
  {
    tag: '尖子生',
    title: '满分冲刺与拓展',
    goal: '训练迁移能力、表达层次和高阶思考。',
    tasks: ['C卷变式迁移', '作文片段升格', '拓展奥赛/竞赛表达题']
  }
];

function getWrongRule(item) {
  return WRONG_CODE_RULES[item.type] || {
    code: 'G1',
    label: '方法未固化',
    train: '先复述解题步骤，再做一道同类变式题。'
  };
}

function buildReviewSchedule(timestamp) {
  ...
  return `建议 ${format(day3)} 复习，${format(day7)} 再测`;
}

function renderSmartWrongbookPanel() {
  const summary = document.getElementById('wrongbookSummary');
  const list = document.getElementById('smartWrongbookList');
  if (!summary || !list) return;

  const wrongList = getWrongAnswers();
  const codeCounts = wrongList.reduce((acc, item) => {
    const rule = getWrongRule(item);
    acc[rule.code] = (acc[rule.code] || 0) + 1;
    return acc;
  }, {});
  const mainCode = Object.entries(codeCounts).sort((a, b) => b[1] - a[1])[0];

  summary.innerHTML = `
    <div class="wrongbook-stat"><strong>${wrongList.length}</strong><span>累计错题</span></div>
    <div class="wrongbook-stat"><strong>${mainCode ? mainCode[0] : '暂无'}</strong><span>主要错因码</span></div>
    <div class="wrongbook-stat"><strong>${wrongList.length ? '3天/7天' : '待生成'}</strong><span>复习提醒</span></div>
  `;

  if (!wrongList.length) {
    list.innerHTML = '<div class="wrongbook-item"><h4>还没有错题</h4><p>完成上方互动练习后，错题会自动进入这里，并生成错因码和复习节奏。</p></div>';
    return;
  }

  list.innerHTML = wrongList.slice(-5).reverse().map((item) => {
    const rule = getWrongRule(item);
    return `
      <article class="wrongbook-item">
        <span class="wrong-code">${rule.code} ${rule.label}</span>
        <h4>${item.type}：${item.question}</h4>
        <p>训练建议：${rule.train}</p>
        <span class="review-pill">${buildReviewSchedule(item.timestamp)}</span>
      </article>
    `;
  }).join('');
}
...
function renderMotivationDashboard() {
  const badgeTitle = document.getElementById('weeklyBadgeTitle');
  const badgeText = document.getElementById('weeklyBadgeText');
  const boardTitle = document.getElementById('answerBoardTitle');
  const boardText = document.getElementById('answerBoardText');
  const curve = document.getElementById('progressCurve');
  if (!badgeTitle || !curve) return;

  const wrongTotal = getWrongAnswers().length;
  const streak = parseInt(localStorage.getItem('checkinStreak') || '0', 10);
  const correct = parseInt(localStorage.getItem('correctCount') || '0', 10);

  badgeTitle.textContent = correct >= 8 ? '本周攻克：拔尖迁移' : correct >= 4 ? '本周攻克：提优提升' : '本周攻克：基础达标';
  badgeText.textContent = `已累计答对 ${correct} 题，连续打卡 ${streak} 天；下一步优先复练 ${wrongTotal || 1} 个薄弱点。`;
  boardTitle.textContent = wrongTotal ? '把错因改成满分步骤' : '满分答案上榜准备中';
  boardText.textContent = wrongTotal ? '订正时补齐“依据、关键词、完整句”，就能进入满分答案墙。' : '完成一次诊断后，会展示孩子可模仿的高分表达。';

  const items = [
    ['基础达标', Math.min(100, 45 + correct * 5)],
    ['提优提升', Math.min(100, 25 + Math.max(0, correct - 3) * 7)],
    ['拔尖迁移', Math.min(100, 12 + Math.max(0, correct - 6) * 8)]
  ];
  curve.innerHTML = items.map(([label, value]) => `
    <div class="progress-bar-row">
      <span>${label} ${value}%</span>
      <div class="progress-bar-track"><div class="progress-bar-fill" style="width:${value}%"></div></div>
    </div>
  `).join('');
}

function renderScenarioPathPanel() {
  const panel = document.getElementById('scenarioPathPanel');
  if (!panel) return;
  panel.innerHTML = SCENARIO_PATHS.map((path) => `
    <article class="scenario-card">
      <span class="scenario-tag">${path.tag}</span>
      <h3>${path.title}</h3>
      <p>${path.goal}</p>
      <ul>${path.tasks.map((task) => `<li>${task}</li>`).join('')}</ul>
    </article>
  `).join('');
}

function initAnswerGrader() {
  const btn = document.getElementById('graderBtn');
  const input = document.getElementById('graderAnswer');
  const result = document.getElementById('graderResult');
  if (!btn || !input || !result) return;

  btn.addEventListener('click', () => {
    const text = input.value.trim();
    if (!text) {
      result.textContent = '先输入孩子答案，再生成诊断建议。';
      return;
    }

    const hasEvidence = /从|文中|第|因为|可以看出|说明/.test(text);
    const longEnough = text.length >= 45;
    const code = hasEvidence ? (longEnough ? 'W2' : 'R3') : 'R2';
    const diagnosis = hasEvidence
      ? longEnough
        ? '答案有依据，可以继续训练表达层次和关键词提炼。'
        : '能找到依据，但表达还不够完整。'
      : '答案缺少原文依据，需要先回文定位。';
    const next = code === 'R2'
      ? '下一步：做2道同类定位题，要求每题先划原文句子再作答。'
      : '下一步：把答案改成“依据 + 分析 + 完整结论”的三段式。';

    result.innerHTML = `<strong>自动错因码：${code}</strong><br>${diagnosis}<br>${next}`;
  });
}
